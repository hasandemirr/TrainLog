...
describe('importBackup — geri yükleme = merge (D27); tohum tek kaynak', () => {
  it('v1 yedeği SEED enjeksiyonuyla göç eder → ad SEED kimliğine çözülür (tek-kaynak kanıtı)', () => {
    ...
    const res = importBackup(seeded(), v1, SEED, { now: 1000, idgen: mkIdgen() });
  ...
  it('ekilmiş durum × yedek: tohum kimlikleri iki tarafta da var → çift yok', () => {
    ...
    const res = importBackup(seeded(), backup, SEED, { now: 1000, idgen: mkIdgen() });
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('v2 yedeği doğrulanır + merge', () => {
    const res = importBackup(emptyState({ now: 0, deviceId: 'a' }), JSON.stringify(seeded()), SEED, { now: 1, idgen: mkIdgen() });
    expect(res.ok).toBe(true);
  });

  it('bozuk JSON / tanınmayan biçim → hata', () => {
    expect(importBackup(seeded(), '{bozuk', SEED, { now: 1, idgen: mkIdgen() }).ok).toBe(false);
    expect(importBackup(seeded(), JSON.stringify({ foo: 1 }), SEED, { now: 1, idgen: mkIdgen() }).ok).toBe(false);
  });
});
